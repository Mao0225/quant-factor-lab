...
def run_suite(p, snapshot, root, seeds, groups=tuple(GROUPS), final_test=False, workers=1):
    ...
    with writer_lock(root):
        if (root / "suite_manifest.json").exists() and digest(read_json(root / "suite_manifest.json")) != digest(manifest):
        ...
        for row in runs:
            if final_test or not row['test_locked']:
        ...
        for group in groups:
            ...
            tested = [r for r in members if not r['test_locked']]
            if tested:
                entry.update(T_seeds=len(tested), T_objective_mean=float(np.mean([r["test_objective"] for r in tested])),
                             T_objective_std=float(np.std([r["test_objective"] for r in tested], ddof=0)))
            summary[group] = entry
