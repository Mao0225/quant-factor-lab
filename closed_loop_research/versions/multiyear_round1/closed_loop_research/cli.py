...
def parser():
    ...
    history.add_argument('--source', type=Path, default=Path('single_factor/data/selected_500_liquid_processed/panel.parquet'))
    history.add_argument('--destination', type=Path, required=True)
    ...
    suite.add_argument('--protocol', type=Path, help='registered run protocol; data identity must match the snapshot')
    suite_report = sub.add_parser('suite-report')
...
def main(argv=None):
    ...
    if args.command == "import-legacy":
        ...
        result = import_history(args.source, args.destination, args.seed)
    elif args.command == 'benchmark':
        from .benchmark import benchmark
        result = benchmark(Protocol.from_dict(read_json(args.data/'protocol.json')), args.data/'snapshot', args.output)
    elif args.command == 'suite-report':
        from .suite_report import build_suite_report
        result = {'report': str(build_suite_report(args.suite).resolve())}
        ...
        p = Protocol.from_dict(read_json(args.protocol or args.data / "protocol.json"))
        full = run_suite(p, args.data / "snapshot", args.output, args.seeds, args.groups, args.final_test, workers=args.workers)
