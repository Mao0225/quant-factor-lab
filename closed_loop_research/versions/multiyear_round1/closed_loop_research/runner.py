...
def run_experiment(p, snapshot_path, root, interrupt_after=None, max_batches_this_call=None, progress=None):
    root = Path(root)
        ...
        while state["batch"] < p.batches:
            ...
            if state["phase"] == "ppo_updated":
                ...
                if progress is not None:
                    progress(dict(batch=state['batch'], pool_version=state['pool_version'], budget=state['budget']))
