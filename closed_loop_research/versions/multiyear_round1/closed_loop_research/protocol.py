...
METHOD_VERSION = "v0.4-kernel-2"
OPERATOR_VERSION = "prefix-causal-history-bound-2"
...
class Protocol:
    ...
    field_lookbacks: tuple = ()
    operator_profile: str = "basic"
    require_feature: bool = False
    compress_artifacts: bool = False
    holdout_status: str = "not_declared"
    ...
    def from_dict(cls, raw):
        ...
        lookbacks = raw["field_lookbacks"]
        raw["field_lookbacks"] = tuple(sorted(lookbacks.items() if isinstance(lookbacks, dict) else lookbacks))
        result = cls(**raw)
        result.validate()
        return result
    ...
    def validate(self):
        ...
        if self.operator_profile not in {"basic", "extended"}:
            raise ValueError("unknown operator profile")
        if self.holdout_status not in {"not_declared", "unseen", "previously_observed_historical"}:
            raise ValueError("unknown holdout status")
        if self.max_lookback is not None and (type(self.max_lookback) is not int or self.max_lookback < 1):
            raise ValueError("max_lookback must be a positive integer")
        if any(k not in self.fields or type(v) is not int or v < 0 for k, v in self.field_lookbacks):
            raise ValueError("invalid field lookback")
        if len(dict(self.field_lookbacks)) != len(self.field_lookbacks):
            raise ValueError("duplicate field lookback")
        if self.max_lookback is not None and max([v for _, v in self.field_lookbacks] or [0]) > self.max_lookback:
            raise ValueError("field history exceeds protocol limit")
        last = None
    ...
    def to_dict(self):
        ...
        raw["field_lookbacks"] = dict(self.field_lookbacks)
        return raw
    ...
    def warmup(self):
        ...
        return self.max_tokens * max(self.windows) + 1 + max([v for _, v in self.field_lookbacks] or [0])

    @property
    def data_digest(self):
        body = {"segments": self.segments, "fields": self.fields, "warmup": self.warmup,
                       "synthetic": self.synthetic, "mode": self.data_mode,
                       "universe": self.universe_note, "execution": self.execution_note}
        if self.field_lookbacks:
            body["field_lookbacks"] = self.field_lookbacks
        return digest(body)
