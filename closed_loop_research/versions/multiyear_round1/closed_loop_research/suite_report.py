...
def build_suite_report(root):
    ...
    for row in suite['runs']:
        ...
        item = dict(group=row['group'], seed=row['seed'], weights=model['weights'],
                    ...
                    ppo_updates=state['ppo_updates'], years=years,
    ...
    html += f'<header><small>CLOSED LOOP / MULTIYEAR RESEARCH</small><h1>500 股 · 多年研究对照</h1><p>{escape(str(p["segments"]))}</p></header><main><p class="notice">{label}静态 500 股样本及旧数据复权口径仍属原型假设。保留全部种子与负结果，三种子的描述统计不能证明 PPO 优于随机搜索。</p>'
    ...
    html += '<section><h2>冻结模型分年结果</h2><p>V 用于模型选择，T 用于历史比较。每段独立建仓；首末不足全年时显示区间收益，不年化冒充年度收益。回撤在所列区间重新计算。</p><table><tr><th>运行</th><th>分段</th><th>年份</th><th>交易日</th><th>净收益</th><th>最大回撤</th><th>费用</th><th>滑点</th></tr>'+''.join(annual)+'</table></section>'
