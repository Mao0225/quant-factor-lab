...
def run_suite(p, snapshot, root, seeds, groups=tuple(GROUPS), final_test=False):
    root = Path(root)
    ...
    with writer_lock(root):
        ...
        for group in groups:
            for seed in seeds:
                ...
                state = run_experiment(protocol, snapshot, run)
                model = select_model(run)
                trials = [t for b in state["completed"] for t in b["trials"]]
                runs.append(dict(group=group, seed=seed, run=str(run.resolve()), protocol_digest=protocol.digest,
                                 selection_objective=model["selection_objective"], frozen_model_id=model["model_id"],
        ...
        result = dict(suite_id=digest(manifest), runs=runs, summary=summary, data_mode=p.data_mode,
                      synthetic=p.synthetic, interpretation="descriptive seed statistics; no significance or method-effectiveness claim")
