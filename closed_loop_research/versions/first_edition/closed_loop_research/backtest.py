...
def backtest(frame, scores, start, end, p):
    frame = frame.sort_values(["date", "code"])
    groups = {pd.Timestamp(d): g.set_index("code").to_dict("index") for d, g in frame.groupby("date", sort=True)}
    signal_groups = {pd.Timestamp(d): g for d, g in scores.groupby("date", sort=True)}
    dates = sorted(groups)
    ...
    for idx, date in enumerate(dates):
        ...
        selected = chosen.code.tolist()
        selections.append(dict(date=stamp, signal_date=str(dates[idx-1].date()),
                               codes=selected, scores=[float(x) for x in chosen.score]))
        target = {}
        for c in selected:
            row = day.get(c)
